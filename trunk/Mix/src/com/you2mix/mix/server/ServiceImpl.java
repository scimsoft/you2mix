...
package com.you2mix.mix.server;
...
public class ServiceImpl extends RemoteServiceServlet implements Service {
  ...
  public Date changeNoteVideo(String noteKey, You2MixVideo video)
	  ...
	  final User user = tryGetCurrentUser(UserServiceFactory.getUserService());
	    ...
	    try {
	      ...
	      Store.Video storeVideo = new Store.Video(video.getYouTubeID(),video.getStartTime(),video.getEndTime());
	      note.setVideo(storeVideo);
	      final Date result = api.saveNote(note).getLastUpdatedAt();
	      tx.commit();
  ...
  public CreateObjectResult createNote(final String surfaceKey, final int x,
    ...
    try {
      ...
      final Store.Note note = new Store.Note(me, x, y, width, height, storeVideo);
  ...
  @Override
  public GetVideoResult getVideos(String videoKey) throws AccessDeniedException {
	  
	    final Store.Api api = store.getApi();
	    try {
//	    	final Video fromCache = cache.getVideo(KeyFactory.stringToKey(videoKey));
//	        if (fromCache != null) {
//	        	return new GetVideoResult(null, fromCache);
//	        }
//	        Video video = toClientVideo(api.getVideo(KeyFactory.stringToKey(videoKey)));
	      return new GetVideoResult(null, null);
	    } finally {
	      api.close();
	    }
  }
